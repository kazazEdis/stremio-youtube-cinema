 ...
 */
/**
 * Age-gated, and nothing else.
 *
 * A gated upload still works for a viewer signed in on YouTube, which is why it
 * is kept and labelled rather than removed when it is the only copy of a film.
 * That reasoning does not extend to anything else that fails to play, so
 * nothing else belongs in this set.
 *
 * `region-blocked` is not here either: it reflects wherever the probe ran, and
 * CI runs in the US while the dev box does not.
 */
export const UNPLAYABLE = new Set(['age-gated']);
...
/**
 * Uploads nobody can play, however they are signed in.
 *
 * These are removed outright, and a film with no other copy leaves the
 * catalogue with them — an entry that always fails is worse for a viewer than
 * no entry, because it costs the click and the trust.
 *
 * `unreachable` needs seeing twice. yt-dlp reports a throttled request and a
 * deleted video in much the same breath, and a single bad minute must not
 * delete a film. `private` and `members-only` are unambiguous from one probe:
 * the video says what it is.
 */
export const DEAD = new Set(['unreachable', 'private', 'members-only']);

export const deadYtIds = db => new Set(
  db.prepare(`SELECT ytId FROM fct_playback
              WHERE (verdict IN ('private','members-only'))
                 OR (verdict = 'unreachable' AND fail_count >= 2)`).all().map(r => r.ytId));
